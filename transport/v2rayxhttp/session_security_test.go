...
func TestSessionAlphabetUniqueEntropy(t *testing.T) {
	for _, table := range []string{strings.Repeat("a", 64), strings.Repeat("ab", 32)} {
		if _, _, err := resolveSessionID(table, "6"); err == nil {
			t.Fatal("duplicate alphabet inflated entropy")
		}
	}
	...
	if table != "ab" {
		t.Fatalf("alphabet must be deduplicated, got %q", table)
	}
	if _, _, err := resolveSessionID("ab", "30"); err == nil {
		t.Fatal("accepted insufficient binary space")
	}
}
...
func TestSessionRandomSource(t *testing.T) {
	...
	if !hasCrypto {
		t.Fatal("session IDs must use crypto/rand")
	}
	for _, decl := range file.Decls {
		fn, ok := decl.(*ast.FuncDecl)
		if !ok || (fn.Name.Name != "newSessionID" && fn.Name.Name != "newUUIDSessionID") {
			continue
		}
		ast.Inspect(fn.Body, func(n ast.Node) bool {
			if id, ok := n.(*ast.Ident); ok && id.Name == "randIntn" {
				t.Error("session IDs use padding PRNG")
			}
			return true
		})
	}
