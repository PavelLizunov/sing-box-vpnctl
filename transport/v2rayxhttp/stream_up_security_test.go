	...
	for _, status := range []int{204, 301, 403, 500} {
		t.Run(strconv.Itoa(status), func(t *testing.T) {
			...
			case err := <-writeUnderTest(conn, []byte("hello")):
				if err == nil || !strings.Contains(err.Error(), "upload status") {
					t.Fatalf("lost upload rejection: %v", err)
				}
			case <-time.After(time.Second):
				t.Fatal("upload rejection left Write blocked")
			}
			...
			case <-time.After(time.Second):
				t.Fatal("upload rejection left Read blocked")
			}
		})
	}
}
...
func TestSplitFailureStopsTimers(t *testing.T) {
	...
	if !readStopped || !writeStopped {
		t.Fatal("terminal failure retained deadline timers")
	}
}
